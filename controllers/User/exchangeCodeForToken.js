  ...
  instagram.exchangeToken(req.body.code).then((body) => {
    ...
    isUserRegistered(body.user.id).then((user) => {
      updateAccessToken(body, user)
        ...
        .catch((error) => responses.returnStatus('Failed to update access token', 403, res));
      }).catch(() => {
        registerUser(body)
          .then((data) => responses.returnData(data[0], res))
          .catch((error) => responses.returnStatus('Failed to register user', 500, res));
      });
  }).catch((error) => responses.returnStatus('Instagram API Error', 422, res));
};
